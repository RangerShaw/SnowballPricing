...
class Snowball:
    def __init__(self, _r, _q, _sigma, _t, KO, KI, margin=1, _Ndays1year=252, _timetype='years', obs_day=21):
        '''
        -------------------------------------------------
    ...
    def MCSolver(self, SAll):
        ...
        return minimize(fun, 0.2, method='TNC').x[0]
    ...
    def Pnf(self, _SAll, coupon, HAVE_KI=False, **kwargs):
        ...
        # s_KO = temp[0][arg_KO]
        # t_KO = temp[1][arg_KO]
        temp = (is_KO.cumsum(axis=1) == 0).sum(axis=1)
        s_KO = np.argwhere(temp <= len(KO_list) - 1).squeeze()
        t_KO = temp[temp <= len(KO_list) - 1]
    ...
    def DeltaHedge(self, SAll, DeltaNoKI, Coupon, **kwargs):
        ...
        if not kwargs['isKI'] and kwargs['isKO']:
            ...
            for i in range(SM):
                ...
                paths.append(cf)
                ko_point.append(SAll[i, kox])
            return np.array([ko_point, paths])
...
def Plot3D(df, name='', cmap=cm.coolwarm):
    x = df.index.tolist()
    y = df.columns.tolist()
    x, y = np.meshgrid(x, y)
    ...
    cbar = fig.colorbar(surface, shrink=0.4, pad=0.1)
    for t in cbar.ax.get_yticklabels():
        t.set_fontsize(15)
    ax.set_xlabel("Spot Price", labelpad=28, fontdict={"size": 15})
    ax.set_ylabel("Day to Maturity", labelpad=20, fontdict={"size": 15})
    ax.set_zlabel(name, labelpad=24, fontdict={"size": 15})
    # ax.invert_xaxis()
...
def PlotBee(df, name):
    color = np.where(df.iloc[:, 1] > 0, 'red', 'green').tolist()
    sns.set(font_scale=2.5)

    joint = sns.JointGrid(x=df.columns[0], y=df.columns[1], data=df, height=3.5)
    joint = joint.plot_joint(plt.scatter, color=color, alpha=0.5, edgecolor='w')
    joint.plot_marginals(sns.histplot, kde=True, bins=30, color='midnightblue', edgecolor='k')
    # joint.set_axis_labels()
    # plt.tick_params(axis="both", labelsize=18)
    joint.fig.suptitle(name, fontsize=30, y=1.0)
    # plt.show()
